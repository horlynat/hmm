...
/**
 * Source unique de géolocalisation IP (ipapi.co, HTTPS — contrairement à
 * ip-api.com dont le plan gratuit refuse le HTTPS avec un 403 "SSL
 * unavailable", bug constaté en prod dans l'ancienne implémentation propre à
 * LoginNotificationHandler). Résultat mis en cache par IP (le quota gratuit
 * ipapi.co se fait rate-limiter vite si on l'appelle en live à chaque vue de
 * /profile/{id} comme avant) et échecs journalisés (au lieu d'être avalés
 * silencieusement) pour rester diagnosticable.
 */
class GeolocationService
{
    ...
    private function fetch(string $ip): ?array
    {
        try {
            ...
            $this->logger->warning('Géolocalisation IP échouée', ['ip' => $ip, 'error' => $e->getMessage()]);

            return null;
        }

        if (!empty($data['error'])) {
            $this->logger->warning('Géolocalisation IP : réponse d\'erreur ipapi.co', [
                'ip' => $ip,
